...
export function renderResilienceModule() {
  ...
  window.togglePaneFullscreen = function(paneId = 'view-manager-resilience') {
    ...
    if (pane) {
      if (pane.classList.contains('view-fullscreen-mode')) {
        ...
        // Clear active modal overlays that might block interactions
  ...
  // Bind tab switcher clicks and update active styling
  ...
  tabs.forEach(tab => {
    ...
    if (btn) {
      ...
      btn.onclick = () => {
        ...
        // Dynamic class updating on click
        tabs.forEach(t => {
          const b = document.getElementById(`btn-res-tab-${t}`);
          if (b) {
            if (t === tab) {
    ...
    <div style="width:100%; height:100%; display:flex; flex-direction:column; justify-content:space-around; padding:10px; gap:8px; box-sizing:border-box; overflow:hidden; background:${bgFill}; border-radius:6px; border: 1px solid ${borderCol};">
      ...
      <div style="display:flex; justify-content:space-between; align-items:center; position:relative; gap:6px; width:100%; overflow:hidden;">
        ${chain.steps.map((step, idx) => `
          ...
          ${idx < chain.steps.length - 1 ? `<div style="font-size:0.75rem; color:${stateColor}; font-weight:800; flex-shrink:0;">➔</div>` : ''}
        `).join('')}
      </div>
