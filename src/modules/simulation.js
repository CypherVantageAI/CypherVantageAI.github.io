...
export function renderSimulationTab(container) {
  ...
  container.innerHTML = `
    <div style="display: flex; flex-direction: column; gap: 20px; width: 100%;">
      ...
      <div style="display: flex; gap: 20px; flex-wrap: wrap; width: 100%;">
        ...
        <div class="dashboard-card" style="flex: 3; min-width: 600px; padding: 15px; margin: 0;">
          ...
          <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 10px; max-height: 330px; overflow-y: auto; padding-right: 4px;">
            ${scenarios.map(sc => {
              const riskScore = sc.likelihood * sc.impactRating;
              const severity = riskScore >= 15 ? 'Critical' : 'High';
              ...
              const badgeBg = severity === 'Critical' ? 'rgba(239, 68, 68, 0.15)' : 'rgba(245, 158, 11, 0.15)';
              ...
              return `
                <div class="scenario-template-card" data-id="${sc.id}" style="border: ${cardBorder}; border-radius: 6px; padding: 10px; cursor: pointer; background: var(--bg-card); transition: all 0.2s ease; position: relative;">
                  <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 6px; gap: 6px;">
                    <span style="font-size: 0.72rem; font-weight: 800; color: var(--text-primary); display: block; line-height: 1.35; flex: 1;">
                      ${escapeHTML(sc.name)}
                    </span>
                    ${isActive ? `<span style="font-size: 0.8rem; line-height: 1; flex-shrink: 0;">🎯</span>` : ''}
                  </div>
                  <p style="font-size: 0.65rem; color: var(--text-secondary); line-height: 1.3; margin-bottom: 8px;">
                    ${escapeHTML(sc.description)}
                  </p>
                  <div style="display: flex; justify-content: space-between; align-items: center; font-size: 0.65rem; color: var(--text-muted); border-top: 1px dashed var(--border-color); padding-top: 6px;">
                    <div style="display: flex; gap: 8px;">
                      <span>Category: <b>${escapeHTML(sc.threatCategory)}</b></span>
                      <span>Likelihood: <b>${sc.likelihood}/5</b></span>
                    </div>
                    <span style="font-size: 0.6rem; padding: 2px 6px; border-radius: 3px; font-weight: 700; background: ${badgeBg}; color: ${badgeColor}; text-transform: uppercase; flex-shrink: 0;">
                      ${severity}
                    </span>
                  </div>
                </div>
        ...
        <div class="dashboard-card" style="flex: none; width: 250px; padding: 15px; margin: 0; display: flex; flex-direction: column; gap: 8px;">
        ...
        <div class="dashboard-card" style="flex: 2; min-width: 480px; padding: 15px; margin: 0; display: flex; flex-direction: column; gap: 15px; border: 1px solid rgba(239, 68, 68, 0.25); background: linear-gradient(135deg, rgba(239, 68, 68, 0.03), rgba(0,0,0,0));">
          <div style="display: flex; justify-content: space-between; align-items: center; border-bottom: 1px dashed var(--border-color); padding-bottom: 6px;">
            ...
            <div style="display: flex; align-items: center; gap: 8px;">
              <span style="font-size: 0.62rem; font-weight: 700; color: var(--text-secondary); background: rgba(255,255,255,0.05); padding: 4px 8px; border-radius: 4px;">
                Active Scenario: ${activeScenario.name}
              </span>
              <button id="btn-run-simulation-test" class="btn btn-primary btn-xs" style="font-weight: 700; background: linear-gradient(135deg, #f43f5e, #eab308); color: #fff; border: none; padding: 4px 10px;">
      ...
      <div class="dashboard-card" style="padding: 15px; margin: 0; display: flex; flex-direction: column; gap: 15px;">
        ...
        <div style="position: relative; width: 100%;">
          <button id="btn-copy-readout" class="btn btn-primary btn-xs" style="position: absolute; top: 10px; right: 10px; z-index: 10; font-weight: 700; opacity: 0.95;">📋 Copy Brief</button>
          <div style="background: rgba(0,0,0,0.15); border: 1px solid var(--border-color); border-radius: 6px; padding: 15px; min-height: 180px; font-family: 'Courier New', Courier, monospace; font-size: 0.72rem; line-height: 1.45; overflow-y: auto; color: var(--text-primary); white-space: pre-wrap;" id="readout-text-panel">${generateExecutiveReadoutText(state, activeScenario, simResults).trim()}</div>
        </div>
    </div>
  `;
  ...
  // Bind Run Scenario Simulation button
  const runSimBtn = document.getElementById('btn-run-simulation-test');
  if (runSimBtn) {
    runSimBtn.onclick = () => {
      // Create new report object
      const reportId = `SIM-${Date.now().toString().slice(-6)}`;
      const newReport = {
        id: reportId,
        title: `Drill Simulation Report: ${activeScenario.name}`,
        timestamp: new Date().toLocaleString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }),
        location: activeScenario.id === 'sim-cloud-outage' ? 'Ashburn DC (Virginia)' : (activeScenario.id === 'sim-ransomware' ? 'Boardman DC (Oregon)' : 'London Hub (UK)'),
        threat: activeScenario.threatCategory || 'Simulated Threat',
        severity: activeScenario.severity || 'High',
        status: 'Mitigated & Closed',
        lossPrevented: simResults.totalRevenueLoss,
        systemsAffected: activeScenario.servicesAffected || ['srv-001'],
        summary: `A structured operational resilience simulation test was executed for the scenario "${activeScenario.name}". Outage analysis determined: ${simResults.servicesAffectedCount} services affected, with estimated customer impact of ${simResults.customersAffected} accounts. Recovery completed in ${simResults.recoveryHours} hours ${simResults.recoveryMinutes} minutes (${simResults.rtoCheck}).`,
        actionItems: [
          `Review disaster recovery playbooks for active scenario elements.`,
          `Validate security control thresholds against target compliance metrics.`
        ]
      };
      ...
      alert(`✅ Simulation run successfully completed!\nReport [${reportId}] added to Simulation Runs Audit Log.`);

      // Render reports page if applicable
