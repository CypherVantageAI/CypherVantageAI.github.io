  ...
  const nodesHtml = filteredNodes.map(node => {
    ...
    return `
      <g class="graph-node" data-id="${node.id}" style="cursor: pointer;">
        ...
        <text class="graph-icon" x="${node.x}" y="${node.y + 4}" text-anchor="middle" style="pointer-events: none;">${icon}</text>
        <text class="graph-label" x="${node.x}" y="${node.y + 24}" fill="var(--text-secondary)" text-anchor="middle" font-weight="600" style="pointer-events: none; text-shadow: 0 1px 2px rgba(0,0,0,0.8);">${node.name.split(': ')[0].slice(0, 18)}</text>
      </g>
    `;
  }).join('');
