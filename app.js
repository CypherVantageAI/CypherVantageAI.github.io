...
window.renderResilienceDashboard = function() {
  ...
  if (detailCard) {
    ...
    const finalSystemsHtml = filteredSystems.map(sys => {
      ...
      return `
        <div class="resilience-system-item clickable-system" style="cursor: pointer; display: flex; align-items: center; justify-content: space-between; background: rgba(255,255,255,0.02); border: 1px solid rgba(255,255,255,0.05); padding: 8px 10px; border-radius: var(--border-radius-sm); margin-bottom: 6px; gap: 8px; transition: background-color 0.2s, border-color 0.2s;" onclick="showSystemDetails('${sys.name}')">
          <div style="min-width: 0; flex: 1;">
            <div style="display: flex; align-items: center; gap: 8px; flex-wrap: wrap;">
              <strong style="font-size: 0.82rem; white-space: normal; word-break: break-word;">${sys.name}</strong>
              <span class="badge ${badgeClass}" style="font-size: 0.6rem; padding: 1px 5px; flex-shrink: 0;">${badgeText}</span>
            </div>
            <span class="text-xs text-secondary" style="display: block; font-size: 0.72rem; white-space: normal; word-break: break-word; margin-top: 2px;">${sys.description || sys.type}</span>
          </div>
          <div class="resilience-system-status ${statusClass}" style="font-size: 0.72rem; font-weight: 600; display: flex; align-items: center; gap: 4px; flex-shrink: 0;">
...
window.stopTlptSimulation = function() {
  ...
  if (wasActive) {
    ...
    state.activityLog.unshift({
      time: 'Just Now',
      text: `📑 <b>TIBER-EU Report Saved:</b> Simulation <b>[${reportId}]</b> completed. Mitigated Loss: ${formatCurrency(state.resilience.lossPrevented)}.`
    });
    ...
    <div style="border: 1px solid rgba(255,255,255,0.06); padding: 20px; border-radius: var(--border-radius-md); background: rgba(0,0,0,0.15);">
      ...
      <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 15px; margin-bottom: 20px;">
        ...
        <div>
          ...
          <span style="font-size: 0.8rem; font-weight: 600; color: #34d399;">💰 ${formatCurrency(rep.lossPrevented || 0)}</span>
        </div>
      </div>
...
window.openDoraIncidentReport = function(source) {
  ...
  if (source === 'active') {
    ...
    if (source && source.startsWith('inc-')) {
      const incident = state.incidents.find(i => i.id === source);
      if (incident) {
        rep = {
          id: incident.id,
          title: incident.title,
          timestamp: `${formatDate(new Date())} 09:12`,
          location: incident.id === 'inc-001' ? 'Tokyo DC (APAC)' : 'Boardman DC (Oregon)',
          threat: incident.classification || 'Critical Incident Outage',
          severity: incident.severity,
          status: incident.status || 'Closed',
          lossPrevented: incident.financialLoss,
          systemsAffected: [incident.serviceAffected],
          summary: incident.rootCause + '. ' + incident.lessonsLearned,
          actionItems: [
            'Conduct root-cause analysis post-mortem.',
            'Validate automatic failover playbooks.',
            'Update DORA compliance register.'
          ]
        };
      }
    } else {
      if (source && state.resilience.reports) {
        rep = state.resilience.reports.find(r => r.id === source);
      }
      if (!rep && state.resilience.reports) {
        const activeIndex = state.resilience.activeReportIndex !== undefined ? state.resilience.activeReportIndex : 0;
  ...
  body.innerHTML = `
    ...
    <table style="width: 100%; border-collapse: collapse; margin-bottom: 20px; font-size: 0.78rem;">
      <tr style="border-bottom: 1px solid rgba(255,255,255,0.08);">
        ...
        <td style="padding: 6px 0; color: #10b981; font-weight: 700; font-size: 0.85rem;">💰 ${formatCurrency(rep.lossPrevented || 0)}</td>
      </tr>
      <tr style="border-bottom: 1px solid rgba(255,255,255,0.08);">
...
window.renderServiceNavigator = function() {
  ...
  // Initialize pagination page limits and controls
  window.filterNavigatorServices(true);
...
window.navigatorCurrentPage = 0;
window.navigatorItemsPerPage = 5;

window.filterNavigatorServices = function(resetPage = true) {
  ...
  const totalPages = Math.ceil(totalItems / window.navigatorItemsPerPage) || 1;

  if (window.navigatorCurrentPage >= totalPages) {
    window.navigatorCurrentPage = totalPages - 1;
  }
  if (window.navigatorCurrentPage < 0) {
    window.navigatorCurrentPage = 0;
  }

  const startIdx = window.navigatorCurrentPage * window.navigatorItemsPerPage;
  const endIdx = startIdx + window.navigatorItemsPerPage;
  ...
  window.renderNavigatorPaginationControls(totalPages);
};

window.renderNavigatorPaginationControls = function(totalPages) {
  const controlsContainer = document.getElementById('navigator-pagination-controls');
  if (!controlsContainer) return;

  controlsContainer.innerHTML = `
    <button id="btn-navigator-prev" class="btn btn-secondary btn-xs" style="padding: 3px 8px; font-size: 0.62rem; height: 22px; font-weight: 700;" ${window.navigatorCurrentPage === 0 ? 'disabled' : ''}>◀ Prev</button>
    <span style="font-family: monospace; font-size: 0.65rem; color: var(--text-muted);">Page ${window.navigatorCurrentPage + 1} of ${totalPages}</span>
    <button id="btn-navigator-next" class="btn btn-secondary btn-xs" style="padding: 3px 8px; font-size: 0.62rem; height: 22px; font-weight: 700;" ${window.navigatorCurrentPage >= totalPages - 1 ? 'disabled' : ''}>Next ▶</button>
  `;

  const btnPrev = document.getElementById('btn-navigator-prev');
  const btnNext = document.getElementById('btn-navigator-next');

  if (btnPrev && window.navigatorCurrentPage > 0) {
    btnPrev.onclick = () => {
      window.navigatorCurrentPage--;
      window.filterNavigatorServices(false);
    };
  }

  if (btnNext && window.navigatorCurrentPage < totalPages - 1) {
    btnNext.onclick = () => {
      window.navigatorCurrentPage++;
      window.filterNavigatorServices(false);
    };
